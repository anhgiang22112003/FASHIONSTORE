...
const CustomerCard = ({ customer, setEditingCustomer, setActivePage }) => {
  const handleEditClick = () => {
    setEditingCustomer(customer) // Truyền full object có id
    ...
    <div className="bg-white p-6 rounded-2xl shadow-lg flex flex-col items-center text-center">
      ...
      <div className="flex justify-between w-full text-sm text-gray-600 mb-4">
        <div className="text-left">
          <p>Tổng đơn hàng:</p>
          <p className="font-bold text-lg text-pink-600">{customer?.orderCount}</p>
        </div>
        <div className="text-right">
          <p>Tổng chi tiêu:</p>
          <p className="font-bold text-lg text-pink-600">{customer?.totalSpent}</p>
        </div>
      </div>
      <p className="text-xs text-gray-400">
        Tham gia: {customer?.createdAt
