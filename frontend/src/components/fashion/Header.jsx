...
const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border backdrop-blur-lg">
      ...
      {/* Mobile Navigation */}
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-card shadow-2xl transform transition-transform duration-300 z-50 md:hidden ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        <div
          className="flex justify-between items-center px-6 py-5 border-b border-border"
          style={{
            background: 'linear-gradient(135deg, #fce7f3 0%, #fbcfe8 100%)'
          }}
        >
          ...
          <div className="space-y-2">
            ...
            <div className="border-t border-border pt-2 mt-2">
              <button
                ...
                className="flex items-center justify-between w-full py-3 px-4 text-foreground hover:bg-accent hover:text-primary font-semibold rounded-xl transition-all"
                style={{
                ...
                <div className="ml-4 space-y-1 mt-2 pb-2">
                  {categories.map((category, index) => (
                    <Link
                      ...
                      className="block py-2.5 px-4 text-foreground/80 hover:bg-pink-50 hover:text-pink-600 rounded-lg transition-all text-sm font-medium relative group"
                      onClick={() => setIsMenuOpen(false)}
                      style={{
